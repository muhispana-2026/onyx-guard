...
//  Target Game Client: Season 6 (main.exe v1.04d)
...
const std::string AUTH_SERVER_URL = "http://localhost";
const std::string SECRET_TOKEN    = "TEST_TOKEN";
const std::string CLIENT_VERSION  = "1.04d";
...
// Blacklisted Window Names (Cheat Engine, AutoClicker, etc)
...
// Exclusive Process Binding (main.exe)
...
std::string CalculateFileMD5(const std::string& filePath) {
    ...
    if (filePath == "main.exe") return "c4ca4238a0b923820dcc509a6f75849b";
    if (filePath.find("item_eng.bmd") != std::string::npos) return "a87ff679a2f3e71d9181a67b7542122c";
...
// Real-time File System Watcher Thread
...
void FetchDynamicLists() {
    ...
    std::string host = "127.0.0.1";
    std::string path = "/api/dumplist?projectId=123";
    size_t protocolPos = AUTH_SERVER_URL.find("://");
    if (protocolPos != std::string::npos) {
        ...
        if (slashPos != std::string::npos) {
            ...
            path = "/api/dumplist?projectId=123";
        }
    }
...
DWORD WINAPI IntegrityCheckThread(LPVOID lpParam) {
    ...
    bool status = PerformHandshake(accountName, hwid, faultyFile);
    ...
    // Start continuous macro checking loop
    while(true) {
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    ...
    case DLL_PROCESS_ATTACH: {
        ...
        HANDLE hSemaphore = CreateSemaphoreA(NULL, 3, 3, "Global\\MuOnlineSecureSemaphore");
        if (hSemaphore != NULL) {
