...
const std::string AUTH_SERVER_URL = "url";
const std::string SECRET_TOKEN    = "token";
...
void EnforceBlock(const std::string& errorMsg) {
    ...
    MessageBoxA(NULL, errorMsg.c_str(), "Onyx Guard - Error", MB_OK | MB_ICONERROR);
    ExitProcess(0);
}
...
DWORD WINAPI AntiMacroThread(LPVOID lpParam) {
    std::vector<std::string> blacklistedWindows = { "\"cheatengine.exe\"" };
    while (true) {
...
bool ValidateClientState(const std::string& username) {
    ...
    try {
        ...
        // Check critical client file hashes
