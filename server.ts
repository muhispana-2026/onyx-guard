  ...
  try {
    ...
    if (existingProjects.length === 0) {
      ...
      await db.insert(config).values({
        ...
        serverUrl: "", // left empty so frontend uses default
...
async function startServer() {
    ...
    try {
      ...
      for (const q of queries) {
        try {
          ...
          results.push(`Error on ${q}: ${e.message || String(e)} | Full: ${JSON.stringify(e, Object.getOwnPropertyNames(e))}`);
        }
      }
  ...
  app.delete("/api/dumps", async (req, res) => {
    try {
      const projectId = getProjectId(req);
      await db.delete(dumps).where(eq(dumps.projectId, projectId));
      res.json({ success: true });
    } catch (e: any) {
